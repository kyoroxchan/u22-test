...
class DbOP:
    ...
    def homePost(self):
        ...
        sql = "SELECT post.* ,user.profile_image FROM post JOIN user ON post.id = user.id ORDER BY post_id DESC;"
        print(sql)
...
def home():
    ...
    try:
        ...
        pointAdd = "off"
        return render_template(
            "sns_home.html", result=result, pointAdd=pointAdd, num=num
        )
...
def post_detail(post_id):
    try:
        ...
        return render_template(
            "sns_post_detail.html", result=result, post_user=post_user
        )
...
def user():
    if "userId" in session:
        ...
        test = {}
        return render_template("sns_login.html", test=test)
...
def point():
    if "userId" in session:
        try:
            ...
            return render_template("sns_point.html", id=id, icon=icon, point=point)
...
def loginCheck():
    try:
        ...
        if not result:
            ...
            return render_template("sns_login.html", err=err, test=test)
        for rec in result:
            print(rec["pass"])
            id = rec["id"]
        ...
        if not test["pass"] == rec["pass"]:
            err = "メールアドレスまたはパスワードが違います"
            return render_template("sns_login.html", result=result, err=err, test=test)
        ...
        dbop.close()
        return render_template(
            "sns_home.html", id=id, result=result, icon=icon, point=point
        )
...
def signupMail():
    ...
    body = render_template("sns_email_template.html", to_email=to_email)

    message = Message(subject=subject, recipients=[to_email], html=body)

    mail.send(message)
    return render_template("sns_signupMail.html", to_email=to_email)
...
def signupSetting(mail):
    ...
    if "name" in session:
        name = session["name"]
        id = session["id"]
    return render_template("sns_signupSetting.html", name=name, id=id)
...
def signupCheck():
    ...
    if not result["pass"] == result["pass2"]:
        ...
        return render_template("sns_signupSetting.html", result=result)
    ...
    try:
        ...
        if result["id"] in idCheck:
            return render_template("sns_signupSetting.html", result=result, err=err)
        session["name"] = result["name"]
        session["id"] = result["id"]
        session["pass"] = result["pass"]
        return render_template("sns_signupCheck.html", result=result, mail=mail)
...
def signupComplete():
    ...
    try:
        ...
        session["userIcon"] = "unknownUser.jpg"
        icon = session["userIcon"]
        cTxt = "登録完了しました"
        return render_template("sns_complete.html", cTxt=cTxt, icon=icon)
...
def signout():
    session.clear()
    cTxt = "ログアウトに成功しました"
    return render_template("sns_complete.html", cTxt=cTxt)
...
def searchArea():
    if "userId" in session:
        icon = session["userIcon"]
        point = session["userPoint"]
        return render_template("sns_search.html", icon=icon, point=point)

    return render_template("sns_search.html")
...
def search():
    ...
    try:
        ...
        return render_template(
            "sns_search.html", resultPost=resultPost, resultUser=resultUser
        )
    ...
    if "userId" in session:
        ...
        if number == "1":
            ...
            try:
                ...
                dbop = DbOP("post")
                res = dbop.point(post_id, resultPoint, user_id, my_id)
                ...
                cTxt = "寄付しました"
                return render_template(
                    "sns_complete.html", cTxt=cTxt, point=point, id=id, icon=icon
                )
...
def upload():
    if "userId" in session:
        ...
        try:
            ...
            # 結果出力処理
            return redirect("/")
            # return render_template("sns_home.html",
            #     result=result,
            #     icon=icon,
            #     id=id,
            #     point=point)
...
def comment(post_id):
    if "userId" in session:
        try:
            ...
            print(commentText)
            dbop = DbOP("comment")
            sql = "'" "','" + post_id + "','" + userId + "','" + commentText + "'"
            print(sql)
            dbop.commentIns(sql)
            dbop.close()
            return redirect("/")
...
def profile():
    ...
    try:
        ...
        return render_template(
            "sns_profileUp.html", icon=icon, point=point, name=name, detail=detail
        )
    except mysql.connector.errors.ProgrammingError as e:
        print("***DB接続エラー***")
        print(type(e))
    ...
    try:
        ...
        if "userId" in session:
            ...
            for poi in userpoint:
                ...
                session["userPoint"] = point
            return render_template(
                "shop_home.html", result=result, icon=icon, point=point, id=id
            )

        return render_template("shop_home.html", result=result)
